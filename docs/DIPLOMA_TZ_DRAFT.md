...
- Spring profile: `local` для одного понятного локального стенда.
- `application.yaml`, `application-local.yaml` и `.env.example`; реальные секреты хранятся только в локальном `.env`.
...
29. Конфигурация локальной разработки должна использовать один Spring profile: `local`; реальные секреты не должны попадать в git.
