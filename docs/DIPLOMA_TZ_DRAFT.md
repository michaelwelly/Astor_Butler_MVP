...
- Spring profiles: `test`, `preprod`, `prod`.
- `application.yaml`, `application-test.yaml`, `application-preprod.yaml`, `application-prod.yaml` для быстрого переключения окружений.
...
29. Конфигурация окружений должна разделяться через Spring profiles: `test`, `preprod`, `prod`.
30. Локальное окружение должно подниматься через Docker Compose: PostgreSQL, Redis, MongoDB, Kafka, Ollama, Prometheus, Grafana и опционально backend app.
...
2. Запустить backend на `test` profile.
