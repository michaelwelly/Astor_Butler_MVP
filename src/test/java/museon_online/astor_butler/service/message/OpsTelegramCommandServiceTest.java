...
class OpsTelegramCommandServiceTest {
    ...
    @Test
    void helpShowsSeedProjectCodesForTeam() {
        Optional<OutgoingMessage> result = commandService.handle(incoming("/ops"), BotState.READY_FOR_DIALOG, "/ops");

        assertThat(result).isPresent();
        assertThat(result.get().text()).contains("VIDEO");
        assertThat(result.get().text()).contains("MED");
        assertThat(result.get().text()).contains("IZI");
        assertThat(result.get().text()).contains("ADS");
        assertThat(result.get().text()).contains("/summary MED");
    }

    @Test
    void acceptsTelegramGroupCommandWithBotMention() {
        when(projectService.listProjects(OpsProjectStatus.ACTIVE, null, 20))
                .thenReturn(List.of(project()));

        Optional<OutgoingMessage> result = commandService.handle(
                incoming("/projects@astor_butler_bot"),
                BotState.READY_FOR_DIALOG,
                "/projects@astor_butler_bot"
        );
    ...
    @Test
    void createsProjectFromTelegramCommand() {
        when(projectService.createProject(argThat(command ->
                command.code().equals("AERIS_LAUNCH")
                        && command.name().equals("AERIS restaurant launch")
                        && command.vertical() == OpsProjectVertical.HORECA
                        && command.owner().equals("michael")
        ))).thenReturn(project());

        Optional<OutgoingMessage> result = commandService.handle(
                incoming("/newproject AERIS_LAUNCH \"AERIS restaurant launch\" HORECA @michael"),
                BotState.READY_FOR_DIALOG,
                "/newproject AERIS_LAUNCH \"AERIS restaurant launch\" HORECA @michael"
        );

        assertThat(result).isPresent();
        assertThat(result.get().text()).contains("project created");
        assertThat(result.get().text()).contains("AERIS_LAUNCH");
        verify(projectService).createProject(argThat(command ->
                command.code().equals("AERIS_LAUNCH")
                        && command.vertical() == OpsProjectVertical.HORECA
                        && command.owner().equals("michael")
        ));
    ...
    @Test
    void schedulesCallFromTelegramCommand() {
        when(projectService.getProjectByCode("AERIS_LAUNCH")).thenReturn(project());
        when(projectService.createCall(argThat(command ->
                command.projectId().equals(77L)
                        && command.title().equals("Launch sync")
                        && command.owner().equals("anna")
                        && command.startsAt() != null
        ))).thenReturn(call());

        Optional<OutgoingMessage> result = commandService.handle(
                incoming("/call AERIS_LAUNCH \"Launch sync\" 24.07 15:00 @anna"),
                BotState.READY_FOR_DIALOG,
                "/call AERIS_LAUNCH \"Launch sync\" 24.07 15:00 @anna"
        );

        assertThat(result).isPresent();
        assertThat(result.get().text()).contains("call scheduled");
        assertThat(result.get().text()).contains("Launch sync");
        verify(projectService).createCall(argThat(command ->
                command.projectId().equals(77L)
                        && command.title().equals("Launch sync")
                        && command.owner().equals("anna")
                        && command.startsAt() != null
    ...
    @Test
    void listsUpcomingCallsForProject() {
        when(projectService.getProjectByCode("AERIS_LAUNCH")).thenReturn(project());
        when(projectService.listUpcomingCalls(77L, 20)).thenReturn(List.of(call()));

        Optional<OutgoingMessage> result = commandService.handle(
                incoming("/calls AERIS_LAUNCH"),
                BotState.READY_FOR_DIALOG,
                "/calls AERIS_LAUNCH"
        );

        assertThat(result).isPresent();
        assertThat(result.get().text()).contains("Smart Solution Ops / calls");
        assertThat(result.get().text()).contains("Launch sync");
        verify(projectService).listUpcomingCalls(77L, 20);
    }

    @Test
    void savesArtifactFromTelegramCommand() {
        when(projectService.getProjectByCode("AERIS_LAUNCH")).thenReturn(project());
        when(projectService.createArtifact(argThat(command ->
                command.projectId().equals(77L)
                        && command.title().equals("Launch deck")
                        && command.type() == OpsArtifactType.PRESENTATION
                        && command.owner().equals("anna")
                        && command.url().equals("https://example.com/deck")
        ))).thenReturn(artifact());

        Optional<OutgoingMessage> result = commandService.handle(
                incoming("/artifact AERIS_LAUNCH \"Launch deck\" https://example.com/deck PRESENTATION @anna"),
                BotState.READY_FOR_DIALOG,
                "/artifact AERIS_LAUNCH \"Launch deck\" https://example.com/deck PRESENTATION @anna"
        );

        assertThat(result).isPresent();
        assertThat(result.get().text()).contains("artifact saved");
        assertThat(result.get().text()).contains("https://example.com/deck");
        verify(projectService).createArtifact(argThat(command ->
                command.projectId().equals(77L)
                        && command.title().equals("Launch deck")
                        && command.type() == OpsArtifactType.PRESENTATION
                        && command.owner().equals("anna")
    ...
    @Test
    void listsArtifactsForProject() {
        when(projectService.getProjectByCode("AERIS_LAUNCH")).thenReturn(project());
        when(projectService.listArtifacts(77L, 20)).thenReturn(List.of(artifact()));

        Optional<OutgoingMessage> result = commandService.handle(
                incoming("/artifacts AERIS_LAUNCH"),
                BotState.READY_FOR_DIALOG,
                "/artifacts AERIS_LAUNCH"
        );

        assertThat(result).isPresent();
        assertThat(result.get().text()).contains("Smart Solution Ops / artifacts");
        assertThat(result.get().text()).contains("Launch deck");
        verify(projectService).listArtifacts(77L, 20);
    }
