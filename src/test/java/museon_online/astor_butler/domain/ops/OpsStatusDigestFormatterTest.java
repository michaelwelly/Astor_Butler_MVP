...
class OpsStatusDigestFormatterTest {
    ...
    void formatsTelegramReadyProjectDashboard() {
        ...
        OpsArtifact artifact = new OpsArtifact(
                101L,
                77L,
                "Launch deck",
        ...
        String text = formatter.format(new OpsProjectDashboard(project, List.of(task), List.of(call), List.of(artifact)));
        ...
        assertThat(text).contains("Upcoming calls");
        assertThat(text).contains("Launch sync");
        assertThat(text).contains("Materials");
        assertThat(text).contains("Launch deck");
    }
}
